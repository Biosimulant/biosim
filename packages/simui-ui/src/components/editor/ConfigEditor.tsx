...
const apiGraphToFlow = (
  ...
  const nodes: Node<ModuleNodeData>[] = graph.nodes.map((n) => {
...
const ConfigEditor: React.FC<ConfigEditorProps> = ({ api, initialConfigPath }) => {
    ...
    const init = async () => {
      try {
        const [registryData, currentConfig] = await Promise.all([
          editorApi.getModules(),
          editorApi.getCurrent(),
        ])
        ...
        if (currentConfig.available && currentConfig.graph) {
          ...
          const files = await editorApi.listFiles()
          setFiles(files)
        ...
        editorApi.listFiles().then(setFiles).catch(console.error)
      }
    }
    init()
  ...
  const loadConfig = useCallback(async (path: string) => {
    try {
      setError(null)
      const graph = await editorApi.getConfig(path)
  ...
  const saveConfig = useCallback(async () => {
    ...
    try {
      ...
      await editorApi.saveConfig(configPath, graph)
      setIsDirty(false)
      setError(null)
    } catch (err) {
  ...
  const applyConfig = useCallback(async () => {
    ...
    try {
      const graph = flowToApiGraph(nodes, edges, meta)
      const result = await editorApi.applyConfig(graph, configPath)
      if (result.ok) {
        setIsDirty(false)
        setError(null)
  ...
  const onPreviewYaml = useCallback(async () => {
    try {
      const graph = flowToApiGraph(nodes, edges, meta)
      const result = await editorApi.toYaml(graph)
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '8px' }}>
              {files.map((f) => (
                <div
                  ...
                  onClick={() => {
                    if (f.is_dir) {
                      editorApi.listFiles(f.path).then(setFiles)
                    } else {
                      loadConfig(f.path)
                    }
                  }}

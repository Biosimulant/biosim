...
export function UiProvider({ children }: { children: React.ReactNode }) {
  ...
  const [controls, setControlsState] = useState<ControlsState>({ duration: 10 })
  ...
  const actions: UiActions = React.useMemo(() => ({
    ...
    setControlsIfUnset: (c) => setControlsState((prev) => {
      const next: ControlsState = { ...prev }
      for (const [key, value] of Object.entries(c)) {
        const current = next[key]
        const hasCurrent = Object.prototype.hasOwnProperty.call(next, key)
        const isUnset =
          !hasCurrent ||
          current === undefined ||
          current === '' ||
...
export function isJsonControl(c: Control): c is Extract<Control, { type: 'json' }> {
  return (c as any).type === 'json'
}
